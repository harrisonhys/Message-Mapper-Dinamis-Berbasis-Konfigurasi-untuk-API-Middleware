...
class TransformLog(Base):
    ...
    @property
    def input_payload(self): return json.loads(self._input_payload or "{}")
    @input_payload.setter
    def input_payload(self, value): self._input_payload = json.dumps(value, ensure_ascii=False)
    @property
    def output_payload(self): return json.loads(self._output_payload or "{}")
    @output_payload.setter
    def output_payload(self, value): self._output_payload = json.dumps(value, ensure_ascii=False)
    @property
    def validation_errors(self): return json.loads(self._validation_errors or "[]")
    @validation_errors.setter
    def validation_errors(self, value): self._validation_errors = json.dumps(value, ensure_ascii=False)
    @property
    def mapping_errors(self): return json.loads(self._mapping_errors or "[]")
    @mapping_errors.setter
    def mapping_errors(self, value): self._mapping_errors = json.dumps(value, ensure_ascii=False)
