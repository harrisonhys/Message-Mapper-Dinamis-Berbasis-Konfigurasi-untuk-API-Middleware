...
def load_partner_config(partner_code: str) -> dict:
    config_path = os.path.join(os.path.dirname(__file__), "..", "mock_partners", f"{partner_code}_config.json")
    with open(config_path, encoding="utf-8") as f: return json.load(f)

def run_dynamic(partner_code: str, payloads: list[dict]) -> dict:
    config = load_partner_config(partner_code); rules = config["mapping_rules"]
    ...
    for p in payloads:
        ...
        results.append({"is_success": is_success, "latency_ms": latency, "error_count": len(all_errors)})
    total = len(payloads)
    return {"approach": "dynamic_mapper", "partner": partner_code, "total": total, "success_count": success_count,
